...
export default function SpeedometerGauge({
  ...
  return (
    <div
      className={`card gauge-card${onClick ? " cursor-pointer hover:shadow-md transition-shadow" : ""}`}
      onClick={onClick}
      title={onClick ? `Click to drill down into ${label}` : undefined}
    >
      <div className="kpi-label" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "4px" }}>
        <span>{label}</span>
        {onClick && <span style={{ fontSize: "9px", color: "#94a3b8", background: "#f1f5f9", padding: "1px 6px", borderRadius: "999px" }}>drill down ↗</span>}
      </div>
